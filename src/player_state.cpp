...
auto player::state_update(const player_state state, const float dt) -> player_state
{
	auto set_direction_if_needed = [&](Vector2 dir) -> void {
		...
		if (dir.y == -1.0f) set_direction(direction::up);
		if (dir.y == 1.0f) set_direction(direction::down);
		if (dir.x == -1.0f) set_direction(direction::left);
		if (dir.x == 1.0f) set_direction(direction::right);
	};
	...
	switch (state)
	{
		case player_state::idle:
			{
				if (is_trying_to_attack()) return attack();
				if (is_trying_to_grab() && is_facing_wall()) return try_pickup_item() ? player_state::lift : player_state::grab;
				if (is_trying_to_move()) return player_state::walk;
			}
			return state;
		...
		case player_state::walk:
			{
				if (is_trying_to_attack()) return attack();
				if (get_terrain_tile_type() == tile_type::chair) return player_state::sit;
				if (get_terrain_tile_type() == tile_type::water) return player_state::swim;
				if (is_trying_to_grab() && is_facing_wall()) return try_pickup_item() ? player_state::lift : player_state::grab;
				if (!is_trying_to_move()) return player_state::idle;
		...
		case player_state::carrystill:
			{
				...
				if (is_trying_to_move()) return player_state::carry;
			}
			return state;
		...
		case player_state::carry:
			{
				...
				if (!is_trying_to_move()) return player_state::carrystill;

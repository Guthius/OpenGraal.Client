...
auto find_sprite_rects(const Texture2D &texture) -> std::vector<Rectangle>
{
	...
	for (int y = 0; y < h; ++y)
	{
		for (int x = 0; x < w; ++x)
		{
			...
			// Found the start of a new sprite (top-left of an unvisited non-red region)
			// Determine width by scanning right until a red pixel or edge
			int max_x = x;
			...
			// Determine height by scanning down while the entire row segment is non-red
			int max_y = y;
			bool row_ok = true;
			...
			// Mark visited region and verify rectangular integrity (optional)

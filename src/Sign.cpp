...
void Sign::Draw(float width, float height) const
{
	if (!_open)
	{
		return;
...
void Sign::DrawFrame(float width, float height) const
{
...
void Sign::DrawLetter(char c, Vector2 &pos) const
{
	for (auto &glyph: Glyphs)
	{
		...
		DrawTextureRec(_texture, glyph.rect, pos, WHITE);

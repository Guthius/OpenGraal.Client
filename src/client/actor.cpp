...
void actor::update_animation(const float dt) {
    if (animation_ == nullptr && !animation_name_.empty()) {
...
void actor::set_appearance(const std::string &head, const std::string &body, const std::vector<std::string> &attributes) {
    if (!head.empty()) {
        animation_state_.head = head;
    }

    if (!body.empty()) {
        animation_state_.body = body;
    }

    for (size_t i = 1; i < attributes.size() && i < animation_state_.attr.size(); ++i) {
        animation_state_.attr[i] = attributes[i];
    }
}

void actor::set_animation(const std::string &name) {
    const auto animation_name = og::shared::to_lower(name);

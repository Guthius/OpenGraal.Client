...
namespace {
    ...
    leap_frame_set frame_set_empty;

    leap_frame_set frame_set_bush = {
    ...
    leap_frame_set frame_set_swamp = {
        {{0, 0, 8},  {0, 8, 0},  {0, 12, 8}},
        {{0, -2, 8}, {0, 8, -2}, {0, 14, 8}},
        {{0, -4, 8}, {0, 8, -4}, {0, 16, 8}},
        {{0, -6, 8}, {0, 8, -6}, {0, 18, 8}},
    ...
    leap_frame_set frame_set_stone = {
    ...
    constexpr auto frame_duration = 0.05f;

    const leap_frame_set &get_leap_frame_set(const leap_effect_type type) {
        switch (type) {
        case leap_effect_type::bush:  return frame_set_bush;
        case leap_effect_type::swamp: return frame_set_swamp;
        case leap_effect_type::stone: return frame_set_stone;
        case leap_effect_type::sign:  return frame_set_sign;
        default:                      return frame_set_empty;
        }
    }
}

leap_effect::leap_effect(const leap_effect_type type, const Vector2 position)
    : texture_(load_texture("sprites.png")),
      type_(type), position_(position),
      frame_set_(get_leap_frame_set(type)) {
}

void leap_effect::update(const float dt) {
    if (!alive_) {
        return;
    }

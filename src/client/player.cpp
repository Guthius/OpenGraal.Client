...
auto player::check_for_level_link_at_at(const Vector2 &position) -> bool {
    ...
    const auto *const link = level->get_link_at(x, y);
    if (link == nullptr) {
        return false;
    }
...
auto player::check_for_sign_at(const Vector2 &position) const -> bool {
    ...
    const auto *const sign = level->get_sign_at(x, y);
    if (sign == nullptr) {
        return false;
    }
...
void player::try_destroy_object_facing(const Vector2 &position) const {
    ...
    if (leap_type != leap_effect_type::none) {
        spawn_leaps(leap_type, Vector2(static_cast<float>(leap_x), static_cast<float>(leap_y)));
    }
}
...
auto player::try_move_from_wall(Vector2 position) -> void {
    ...
    auto on_link = [&](const Vector2 &p) {
        const auto &level = get_current_level();

        return level != nullptr && level->get_link_at(p.x, p.y) != nullptr;
    };

    if (collides(position)) {
        static constexpr std::array search_dirs = {
            Vector2{0,  0 },
            Vector2{1,  0 },
            Vector2{-1, 0 },
            Vector2{0,  1 },
        ...
        bool resolved = false;
        for (const auto links_allowed : {false, true}) {
            for (int r = 1; r <= 16 && !resolved; ++r) {
                for (const auto &[x, y] : search_dirs) {
                    ...
                    if (!collides(candidate) && (links_allowed || !on_link(candidate))) {
                        position = candidate;
                        resolved = true;
                        break;
                    }
...
auto player::can_jump(const Vector2 &position) const -> bool {
    ...
    if (const auto tile = get_tile_facing(); (tile & tile_type::jump) == 0) {
        return false;
    }

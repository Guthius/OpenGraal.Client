...
void gui_control_profile::draw_frame(const Rectangle &rect, const bool hot, const bool disabled) const {
    ...
    const auto thickness = std::max(0, border_thickness);
    if (border_style == gui_border_style::none || thickness == 0) {
        return;
    }

    const auto white = with_alpha(WHITE, transparency_);
    const auto black = with_alpha(BLACK, transparency_);
    const auto color_border_na = with_alpha(border_color_na, transparency_);

    const auto edges = [&](const Color top_left, const Color bottom_right, const int inset) {
        DrawRectangle(x + inset, y + inset, width - (inset * 2), thickness, top_left);
        DrawRectangle(x + inset, y + inset, thickness, height - (inset * 2), top_left);
        DrawRectangle(x + inset, y + height - inset - thickness, width - (inset * 2), thickness, bottom_right);
        DrawRectangle(x + width - inset - thickness, y + inset, thickness, height - (inset * 2), bottom_right);
    };

    switch (border_style) {
    case gui_border_style::single:
        edges(color_border, color_border, 0);
        break;

    case gui_border_style::rectangle:
        edges(white, color_border_na, 0);
        break;

    case gui_border_style::lowered:
        edges(color_border_na, white, 0);
        break;

    case gui_border_style::raised:
        edges(white, black, 0);
        edges(color_border, color_border, thickness);
        break;

    case gui_border_style::none:
        break;
    }
}

auto gui_control_profile::measure_text(const std::string &text) const -> Vector2 {
    if (!IsFontValid(font_)) {
        return {0, font_size_};
    }

    return MeasureTextEx(font_, text.c_str(), font_size_, 1.0f);
}

void gui_control_profile::draw_text(const std::string &text, const Rectangle &rect, const alignment alignment, const std::optional<Color> color) const {
    if (!IsFontValid(font_)) {
        return;
    }
    ...
    DrawTextEx(font_, text.c_str(), dest, font_size_, 1.0f, with_alpha(color.value_or(font_color), transparency_));
}

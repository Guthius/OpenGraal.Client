...
namespace {
    ...
    void draw_frame(const Texture2D &texture_, const float width, const float height) {
        ...
        const auto border_x = width - 32;
        if (border_x > 0) {
            DrawTexturePro(texture_, {64, 192, 16, 32}, {16, 0, border_x, 32}, {0, 0}, 0, WHITE);
            DrawTexturePro(texture_, {80, 192, 16, 32}, {16, height - 32, border_x, 32}, {0, 0}, 0, WHITE);
        }

        const auto border_y = height - 64;
        if (border_y > 0) {
            DrawTexturePro(texture_, {96, 192, 16, 32}, {0, 32, 16, border_y}, {0, 0}, 0, WHITE);
            DrawTexturePro(texture_, {112, 192, 16, 32}, {width - 16, 32, 16, border_y}, {0, 0}, 0, WHITE);
        }

        if (border_x > 0 && border_y > 0) {
            DrawRectangle(
                16, 32,
                static_cast<int>(border_x),
                static_cast<int>(border_y),
                background_color);
        }
    }

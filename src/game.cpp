...
game::game()
{
	file_manager::BuildFileTable("levels");
	...
	level_ = new LevelInfo(
		level_manager::Get("onlinestartlocal.graal"),
		tileset_manager::Get("pics1.png"),
		0, 0);

	state_ = texture_manager::Get("state.png");
...
{
	const auto level = level_manager::Get(level_name);
...
{
	if (const auto sound = sound_manager::Get("crush.wav"); IsSoundValid(sound))
	{
		PlaySound(sound);
	}
